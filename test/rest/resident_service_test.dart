...
/// when it is up, and what has to be reachable from here for it to serve anything real.
///
...
/// whether an option was typed. A machine asked for one could be given the other. Here the word,
/// the options, the refusals and the command a unit file carries are all one statement.
///
/// **AND WHAT IT NEEDS TO EXIST AT ALL.** Answering `GET /programs` and starting a run means
/// resolving a program's step names against a registry, and Dart ahead of time loads no code that
/// was not compiled in — so a service composed where no plugin is reachable serves an empty
/// catalogue and refuses every program a machine has. The last group holds that this tree is the
/// one that can compose it: the manifest reaches the plugins, and a manifest that reached none is
/// reported.
void main() {
  ...
  group('this tree can compose the registry the surface serves', () {
    test('THE MANIFEST AS IT STANDS reaches the plugins this product carries', () {
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'a service composed where no plugin is reachable answers GET /programs with an empty '
            'catalogue and refuses every program a machine has — and it does so at the first '
            'request rather than at start-up, so the machine looks installed',
      );
    });

    test('THE PLANTED DEFECT: a manifest reaching no plugin is reported as reaching none', () {
      expect(
        ...
        isEmpty,
        reason:
            'the framework and the rule set carry no step between them, so a rule that counted '
            'either as a registry would pass the manifest test above without a plugin in sight',
      );
    });

    for (final String planted in <String>['ansiwise_host', 'ansiwise_kubernetes']) {
      test('THE PLANTED INNOCENT: a manifest reaching $planted is named as reaching it', () {
        expect(
          registryReachingDependencies(
            'name: ansiwise_cli\n'
            'dependencies:\n'
