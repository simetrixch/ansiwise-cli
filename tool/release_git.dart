...
///
/// The same split tool/gate/package_gate.dart makes against tool/gate/real_dart_toolchain.dart: what
/// the release program DECIDES is one thing and starting a program on this operating system is
/// another. It is what lets the deciding half be driven by a check — including the half that pushes
/// — on a machine that starts no git and reaches no remote, and what was asked for is then a list of
/// argument lists in the order it was asked. That list is the evidence for the two claims of this
/// program: the screen writes nothing, and a version that is typed pushes exactly one tag.
///
...
///
/// The working directory is the one the program was started in, which is the repository root here,
/// so `git ls-remote` reads the remote of THIS checkout rather than of whatever directory a shell
/// happened to be in.
final class GitOnThisMachine implements Git {
  /// The git on the PATH.
  const GitOnThisMachine();

  @override
  Future<GitAnswer> run(List<String> arguments) async {
    try {
      final ProcessResult result = await Process.run('git', arguments);
      return GitAnswer(
