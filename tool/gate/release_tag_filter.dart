...
/// build, no GitHub Release, no file for hostyour-manager's place-ansiwise to fetch. The patterns
/// are read out of that file every time tool/release.dart runs, and what a person types is held
/// against them. A grammar spelled a second time in Dart would be a second answer to "may this be
/// released", and the day the workflow's filter was widened or narrowed the two would disagree with
/// nothing in this repository able to notice — which is why ansiwise-client's tool/release_version.dart
...
/// The workflow whose `on.push.tags` decides which tags start a release, as this repository holds it.
...
/// the gate that resolves this tree is a program of the same directory and has to start on a clone
/// where no package has been resolved. What is walked is three keys and a list of items under them,
/// and a file that does not carry them answers with nothing, which [TagFilter.unreadable] turns into
/// a refusal rather than into a filter that accepts everything.
